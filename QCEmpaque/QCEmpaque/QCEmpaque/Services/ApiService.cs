namespace QCEmpaque.Services
{
    ...
    public class ApiService
    {
        ...
        public async Task<Response> GetList1<T>(string urlBase, string servicePrefix, string controller)
        {
            try
            {
                var client = new HttpClient();
        ...
        public async Task<Response> MyGetList<T>(string urlbase, string url)
        {
            try
            {
        ...
        public async Task<Response> MyPost<T>(string urlbase, string url, T model)
        {
            try
            {
                var data = JsonConvert.SerializeObject(model);
                var contentenido = new StringContent(data, Encoding.UTF8, "application/json");
                HttpClient client = new HttpClient();
                client.BaseAddress = new Uri(urlbase);
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage response;
                response = await client.PostAsync(url, contentenido);
                if (!response.IsSuccessStatusCode)
                {
                    return new Response
                    {
        ...
        public async Task<Response> Login<T>(string urlbase, string url, Login Login)
        {
            try
            {
                ...
                //client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                //HttpResponseMessage response;
                var response = await client.PostAsync(url, contentenido);
                if (!response.IsSuccessStatusCode)
                {
                    return new Response
                    {
